    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Set spending limits per category and track your progress.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-20 bg-white border border-slate-200/80 rounded-2xl text-center text-slate-500 max-w-xl mx-auto flex flex-col items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="font-semibold text-slate-900 text-sm">No Budgets Configured</p>
            <p className="text-xs text-slate-500 mt-1">Start tracking your spending by configuring a monthly or yearly budget limit.</p>
          </div>
          <button
            onClick={openCreateForm}
            className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold rounded-xl border border-slate-200 transition-all cursor-pointer"
          >
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 relative text-slate-800">
            <h3 className="text-lg font-bold text-slate-900 mb-4">
              {editingBudget ? "Edit Budget Limit" : "Configure Budget Limit"}
            </h3>

            {formError && (
              <div className="p-3 bg-rose-50 border border-rose-100 text-rose-600 rounded-xl text-xs mb-4">
                {formError}
              </div>
            )}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-900 rounded-xl px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all disabled:opacity-50"
                >
