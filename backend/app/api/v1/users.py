...
async def get_my_profile(
    ...
    logger.debug("Profile lookup — clerk_id=%s", clerk_id)
    user = await user_service.get_user_by_clerk_id(db, clerk_id)
    if not user:
        logger.warning("User not found in DB — clerk_id=%s", clerk_id)
        raise HTTPException(
...
async def get_my_preferences(
    ...
    logger.debug("Fetching preferences — user_id=%s", user_id)
...
async def upsert_my_preference(
    ...
    logger.info("Upserting preference — user_id=%s key=%s", user_id, payload.key)
...
async def delete_my_preference(
    ...
    logger.info("Deleting preference — user_id=%s key=%s", user_id, key)
    deleted = await memory_service.delete_preference(db, user_id, key)
    if not deleted:
        logger.warning("Preference key not found — user_id=%s key=%s", user_id, key)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
